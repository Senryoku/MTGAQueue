...
export const socket = {
	install: function (Vue, options) {
		function connectWS() {
			const socket = new WebSocket(`ws://${ServerHost}`);
			Vue.config.globalProperties.$socket = {
				ws: socket,
				socketAutoReconnect: true,
				onReconnect: [],
				addOnReconnect: function (f) {
					this.onReconnect.push(f);
				},
			...
			socket.addEventListener("close", function closeSocket(event) {
				...
				if (Vue.config.globalProperties.$socket.socketAutoReconnect)
					setTimeout(() => {
						for (let f of Vue.config.globalProperties.$socket
							.onReconnect)
							f();
						connectWS();
					}, 500);
		...
		Vue.config.globalProperties.$socketEmit = (command, data) => {
			Vue.config.globalProperties.$socket.ws.send(
